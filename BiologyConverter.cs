...
namespace Tools.Converter.Biology
{
    ...
    {
        public static void ReadDir(string path, string separator, out double[,] matrix, out bool[] rev, out String[] metabolite_names, out String[] reaction_names, out LinkedList<FluxPattern> forbidden_combinations)
        {
            ...
            forbidden_combinations = new LinkedList<FluxPattern>();
            try
            {
                LinkedList<bool[]> cycles = Reader.ReadBoolFile(path + "\\cycles.txt", " ");
                foreach (bool[] cycle in cycles)
                    forbidden_combinations.AddLast(new FluxPattern(cycle, false));
            }
            catch (Exception) {}
        }
